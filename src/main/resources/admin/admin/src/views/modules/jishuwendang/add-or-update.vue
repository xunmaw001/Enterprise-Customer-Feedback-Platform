<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.wendangtupian" label="文档图片" prop="wendangtupian">
          <file-upload
          tip="点击上传文档图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.wendangtupian?ruleForm.wendangtupian:''"
          @change="wendangtupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.wendangtupian" label="文档图片" prop="wendangtupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.wendangtupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.jishuwendang" label="技术文档" prop="jishuwendang">
          <file-upload
          tip="点击上传技术文档"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.jishuwendang?ruleForm.jishuwendang:''"
          @change="jishuwendangUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.jishuwendang" label="技术文档" prop="jishuwendang">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.jishuwendang)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.wendangmingcheng = true;
          ...
          if(o=='wendangtupian'){
            this.ruleForm.wendangtupian = obj[o];
	    this.ro.wendangtupian = true;
            continue;
          }
          if(o=='jishuwendang'){
            this.ruleForm.jishuwendang = obj[o];
	    this.ro.jishuwendang = true;
            continue;
          }
	...
	if(this.ruleForm.wendangtupian!=null) {
		this.ruleForm.wendangtupian = this.ruleForm.wendangtupian.replace(new RegExp(this.$base.url,"g"),"");
	}


	if(this.ruleForm.jishuwendang!=null) {
		this.ruleForm.jishuwendang = this.ruleForm.jishuwendang.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jishuwendang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jishuwendangCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    wendangtupianUploadChange(fileUrls) {
	this.ruleForm.wendangtupian = fileUrls;
	this.addEditUploadStyleChange()
    },
    jishuwendangUploadChange(fileUrls) {
	this.ruleForm.jishuwendang = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
